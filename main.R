...
# Outputs:
#       out (list) w/ following components
...
df_pred <- vector("list", 14)

for (h in 1:14) {
  ud_h <- alpha_h[, h] + expm::`%^%`(A, h) %*% (u_D - alpha_D)
  ud_h <- as.vector(ud_h)
  indx_f <- (1:len_i - 1) * 7 * len_t + (wd_h[h] - 1) * len_t
